    ...
    <template #input>
      ...
      <template v-else>
        <component
          ...
          :style="getInputStyle()"
          v-model="
            Array.isArray($parent.$attrs.data[$attrs.name]) && ['text'].includes($attrs.editors)
              ? $parent.$attrs.data[$attrs.name][1]
              : $parent.$attrs.data[$attrs.name]
          "
          :options="
...
export default {
  ...
  components: {
    ...
    Radio: {
      template: `
                <van-radio-group v-model="$attrs.value" v-on="$listeners" class="cq_radio">
                  <van-radio
    ...
    Checkbox: {
      template: `
                <van-checkbox-group  v-on="$listeners" v-model="$attrs.value" class="cq_radio">
                  <van-checkbox
                    v-for="(item, index) in $attrs.options"
                    :key="item.name"
                    :name="item.name"
