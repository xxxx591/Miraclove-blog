cqjs.editor('radio', {
    getTpl: function () {
        let id = cqjs.nextId();
        let me = this, html = `<div class="form-radio" id="${this.getId()}">`;
    ...
    setValue: function (v) {
        if (v != this.getValue()) {
            if (v) {
                this.dom.find("input[value=" + v + "]").prop('checked', true).trigger('change');
            } else {
                this.dom.find("input").prop('checked', false).trigger('change');
            }
        }
    },
});

cqjs.searchEditor('radio', {
