...
(function () {
    ...
    String.prototype.t = function () {
        ...
        if (!top.window.langData) {
            let lang = cqjs.web.cookie('ctx_lang');
            let data = localStorage.getItem('lang-' + lang);
            top.window.langData = data ? JSON.parse(data) : {};
        }
        let value = top.window.langData[str];
    ...
    cqjs.msg = {
        ...
        error: function (err, opt) {
            if (typeof err === 'string') {
            ...
            } else if (err.code === 1000 || err.data === undefined) {
                //ValidationException
                $(document).Toasts('create', $.extend({
                    class: 'msg bg-danger',
                    title: '验证不通过'.t(),
                ...
                console.error(err);
                //其它类型错误
                let msg = `<div style="min-width:200px">${err.message ? err.message.replaceAll('\r\n', '<br/>') : err.message}</div>`;
                //非UserException
        ...
        confirm: function (options) {
            let opt = $.extend({
                    title: '确认'.t(),
                    content: '',
                    cancelButton: `<button class="btn btn-default cancel">${'取消'.t()}</button>`,
    ...
    cqjs.changeXml = function (xml = '') {
        ...
        return xml
          .replaceAll("on-", "v-on:")
          .replaceAll("bind-", "v-bind:")
      }
      cqjs.modelFields = {
        "mes.solder_paste_recovery_pda" : {
            reel_id: {
              name: "reel_id",
              type: "char",
              label: "ReelID",
