cqjs.editor('boolean', {
    ...
    getAllowNullTpl: function () {
        return `<select class="form-control" id="${this.name + '-' + cqjs.nextId()}">
                    <option value=""></option>
    ...
    setValue: function (v) {
        ...
        if (me.allowNull) {
            ...
            if (me.getValue() !== val) {
                return me.dom.children('select').val(val).trigger('change');
            }
        } else {
            if (me.getValue() !== v) {
                me.dom.children('input').prop("checked", v === true).trigger('change');
            }
        }
    }
});

cqjs.searchEditor('boolean', {
